...
use user_lib::{
    close, connect, recvfrom, sendto, sleep, socket, tls_close, tls_connect, tls_read, tls_write,
};
...
pub fn main_with_args(argc: usize, argv: *const usize) -> i32 {
    if argc < 2 {
        println!("usage: httpsget <host-or-url> [path] [dns-ip]");
        println!("example: httpsget example.com /");
    ...
    https_get(host, path, ip)
}

fn https_get(host: &str, path: &str, ip: u32) -> i32 {
    ...
    let addr = SockAddrIn::new(ip, HTTPS_PORT);
    let ret = connect(
        fd,
    ...
    if !append(&mut req, &mut n, b"GET ")
        ...
        || !append(
            &mut req,
            &mut n,
            b"\r\nConnection: close\r\nUser-Agent: kairix-httpsget/0.1\r\n\r\n",
        )
    {
