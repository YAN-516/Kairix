...
const REAL_GCC: &str = "/usr/bin/gcc";
...
pub fn main_with_args(argc: usize, argv: *const usize) -> i32 {
    ...
    for index in 1..argc {
        let Some(arg) = argv_str(argv, index) else {
            println!("gcc: invalid argument {}", index);
            return 2;
        };
        args.push(arg);
    }

    let env = ["PATH=/usr/bin:/bin:/sbin:/musl", "HOME=/", "TMPDIR=/tmp"];
    let ret = execve(REAL_GCC, &args, &env);
    println!("gcc: failed to execute {}: {}", REAL_GCC, ret);
    127
}
...
fn print_help() {
    println!("Usage: gcc [options] file...");
    println!("Options:");
