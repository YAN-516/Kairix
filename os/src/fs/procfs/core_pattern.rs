...
const CORE_PATTERN_VALUE: &str = "core\n";

pub struct CorePatternFile {
    inner: Mutex<FileInner>,
}

impl CorePatternFile {
...
impl File for CorePatternFile {
    ...
    fn read(&self, mut buf: UserBuffer) -> SysResult<usize> {
        ...
        let data = CORE_PATTERN_VALUE.as_bytes();
...
pub struct CorePatternDentry {
    inner: DentryInner,
}

impl CorePatternDentry {
    pub fn new(name: &str, parent: Option<Arc<dyn Dentry>>) -> Arc<Self> {
        let parent_weak = parent.as_ref().map(|p| Arc::downgrade(p));
        Arc::new_cyclic(|_me: &Weak<CorePatternDentry>| Self {
            inner: DentryInner::new(name, parent_weak),
        })
    }
}

impl Dentry for CorePatternDentry {
    ...
    fn open(self: Arc<Self>, _flags: OpenFlags, _mode: InodeMode) -> SysResult<Arc<dyn File>> {
        Ok(Arc::new(CorePatternFile::new(self)))
    }
}

pub struct CorePatternInode {
    inner: InodeInner,
}

impl CorePatternInode {
    pub fn new() -> Self {
        Self {
...
impl Inode for CorePatternInode {
