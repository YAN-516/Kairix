...
pub struct PipeMaxSizeFile {
    inner: Mutex<FileInner>,
}

impl PipeMaxSizeFile {
...
pub struct PipeMaxSizeDentry {
    inner: DentryInner,
}

impl PipeMaxSizeDentry {
    pub fn new(name: &str, parent: Option<Arc<dyn Dentry>>) -> Arc<Self> {
        let parent_weak = parent.as_ref().map(|p| Arc::downgrade(p));
        Arc::new_cyclic(|_me: &Weak<PipeMaxSizeDentry>| Self {
            inner: DentryInner::new(name, parent_weak),
        })
    }
}

impl Dentry for PipeMaxSizeDentry {
    ...
    fn open(self: Arc<Self>, _flags: OpenFlags, _mode: InodeMode) -> SysResult<Arc<dyn File>> {
        Ok(Arc::new(PipeMaxSizeFile::new(self)))
    }
}
...
impl Inode for PipeMaxSizeInode {
