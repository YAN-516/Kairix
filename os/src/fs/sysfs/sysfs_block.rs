...
pub struct SysfsTextFile {
    inner: Mutex<FileInner>,
    content: &'static str,
}

pub struct SysfsNodeMeminfoFile {
    inner: Mutex<FileInner>,
    content: String,
}

impl SysfsNodeMeminfoFile {
    pub fn new(dentry: Arc<dyn Dentry>) -> Self {
        let stats = crate::mm::frame_stats();
        let total_kb = stats.total_pages.saturating_mul(PAGE_SIZE) / 1024;
        let free_kb = stats.free_pages.saturating_mul(PAGE_SIZE) / 1024;
        Self {
            inner: Mutex::new(FileInner {
                offset: 0,
                dentry,
...
impl File for SysfsNodeMeminfoFile {
    fn get_fileinner(&self) -> MutexGuard<'_, FileInner> {
        self.inner.lock()
    }
...
pub struct SysfsNodeMeminfoDentry {
    inner: DentryInner,
}

impl SysfsNodeMeminfoDentry {
    pub fn new(name: &str, parent: Option<Arc<dyn Dentry>>) -> Arc<Self> {
        let parent_weak = parent.as_ref().map(|p| Arc::downgrade(p));
        Arc::new_cyclic(|_me| Self {
            inner: DentryInner::new(name, parent_weak),
...
impl Dentry for SysfsNodeMeminfoDentry {
    fn get_dentryinner(&self) -> &DentryInner {
        &self.inner
    }
    ...
    fn open(self: Arc<Self>, _flags: OpenFlags, _mode: InodeMode) -> SysResult<Arc<dyn File>> {
        Ok(Arc::new(SysfsNodeMeminfoFile::new(self)))
    }
}

impl SysfsTextFile {
    pub fn new(dentry: Arc<dyn Dentry>, content: &'static str) -> Self {
        Self {
            inner: Mutex::new(FileInner {
                offset: 0,
                dentry,
...
pub struct SysfsTextDentry {
    inner: DentryInner,
    content: &'static str,
}

impl SysfsTextDentry {
    pub fn new(name: &str, parent: Option<Arc<dyn Dentry>>, content: &'static str) -> Arc<Self> {
        let parent_weak = parent.as_ref().map(|p| Arc::downgrade(p));
        Arc::new_cyclic(|_me| Self {
            inner: DentryInner::new(name, parent_weak),
            content,
...
impl Dentry for SysfsTextDentry {
    fn get_dentryinner(&self) -> &DentryInner {
        &self.inner
    }
    fn name(&self) -> &str {
    ...
    fn open(self: Arc<Self>, _flags: OpenFlags, _mode: InodeMode) -> SysResult<Arc<dyn File>> {
        Ok(Arc::new(SysfsTextFile::new(self.clone(), self.content)))
    }
}

pub struct SysfsTextInode {
    inner: InodeInner,
}
...
impl SysfsTextInode {
    pub fn new(size: usize) -> Self {
        Self {
            inner: InodeInner::new(inode_alloc(), size, InodeMode::FILE, 0),
        }
    }
}

impl Inode for SysfsTextInode {
    fn get_mode(&self) -> InodeMode {
        self.inner.mode
    }
    fn get_size(&self) -> usize {
