...
pub fn create_fanotify_file(
    flags: u32,
    event_f_flags: u32,
    unprivileged: bool,
    owner_pid: usize,
) -> SysResult<Arc<FanotifyFile>> {
...
pub fn register_fanotify_file(file: &Arc<FanotifyFile>) -> SysResult<()> {
...
pub fn fanotify_init_cloexec(flags: u32) -> bool {
    flags & FAN_CLOEXEC != 0
}

pub fn fanotify_mark_needs_target(flags: u32) -> bool {
    flags & FAN_MARK_FLUSH == 0
}

pub fn fanotify_mark_file(
    fanotify_file: Arc<FanotifyFile>,
    flags: u32,
    mask: u64,
    target: Option<Arc<dyn Dentry>>,
) -> SysResult<()> {
    ...
    let target = target.ok_or(SysError::EFAULT)?;
...
pub fn fanotify_fdinfo(file: &Arc<dyn File + Send + Sync>) -> Option<String> {
    fanotify_file_from_file(file).map(|file| file.fdinfo())
}
...
pub fn fanotify_file_from_file(file: &Arc<dyn File + Send + Sync>) -> Option<Arc<FanotifyFile>> {
