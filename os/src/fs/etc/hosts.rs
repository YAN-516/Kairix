...
impl File for HostsFile {
    ...
    fn read(&self, mut buf: UserBuffer) -> SysResult<usize> {
        ...
        static CONTENT: &[u8] = b"127.0.0.1\tlocalhost localhost.localdomain\n\
...
impl HostsDentry {
    pub fn new(name: &str, parent: Option<Arc<dyn Dentry>>) -> Arc<Self> {
        ...
        Arc::new_cyclic(|_me: &Weak<HostsDentry>| Self {
            inner: DentryInner::new(name, parent_weak),
        })
    }
}

impl Dentry for HostsDentry {
    ...
    fn open(self: Arc<Self>, _flags: OpenFlags, _mode: InodeMode) -> SysResult<Arc<dyn File>> {
        Ok(Arc::new(HostsFile::new(self)))
    }
}

/// /etc/hosts 的 inode。
pub struct HostsInode {
    inner: InodeInner,
}
