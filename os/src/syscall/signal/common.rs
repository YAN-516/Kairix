...
impl LinuxSigInfo {
    fn from_siginfo(info: SigInfo) -> Self {
        ...
        if let Some(address) = info.si_addr {
            // The sigfault payload starts at byte 16 of siginfo_t.
            result._pad[4..12].copy_from_slice(&address.to_ne_bytes());
        } else {
...
pub(super) fn write_linux_siginfo(frame: &mut [u8], signal: Signal, info: Option<&SigInfo>) {
    debug_assert!(frame.len() >= 128);
    let Some(info) = info else {
        frame[0..4].copy_from_slice(&signal.as_i32().to_ne_bytes());
        return;
    };

    frame[0..4].copy_from_slice(&info.si_signo.to_ne_bytes());
    frame[4..8].copy_from_slice(&info.si_errno.to_ne_bytes());
    frame[8..12].copy_from_slice(&info.si_code.to_ne_bytes());
    if let Some(address) = info.si_addr {
        frame[16..24].copy_from_slice(&address.to_ne_bytes());
    } else {
        frame[16..20].copy_from_slice(&info.si_pid.to_ne_bytes());
        frame[20..24].copy_from_slice(&info.si_uid.to_ne_bytes());
        frame[24..28].copy_from_slice(&info.si_value.to_ne_bytes());
    }
}
...
/// Deliver a kernel-generated synchronous memory fault to the thread that
/// executed the faulting instruction. Linux does not place SIGSEGV/SIGBUS
/// generated by a hardware exception in the process-wide pending queue.
pub fn deliver_fault_signal(
    ...
    debug_assert!(matches!(signal, Signal::SigSegv | Signal::SigBus));
    debug_assert!(si_code > 0);

    task.inner_exclusive_access().blocked_signals.remove(signal);
    {
        let mut inner = process.inner_exclusive_access();
        inner.blocked_signals.remove(signal);
        let mut handlers = inner.signals_handler.lock();
        if handlers.get(signal).is_ignored() {
            // A hardware-generated synchronous fault cannot be discarded and
            // retried forever. Linux forces the default disposition when such
            // a signal was ignored.
            let _ = handlers.reset(signal);
        }
    }
