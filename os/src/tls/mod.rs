...
mod imp {
    ...
    impl ServerVerifier for InsecureServerVerifier {
        fn verify_identity(&self, _identity: &ServerIdentity<'_>) -> Result<PeerVerified, rustls::Error> {
    ...
    fn complete_handshake(conn: &mut ClientConnection, io: &mut TcpIo) -> Result<(), SysError> {
        ...
        while conn.is_handshaking() {
            ...
            match conn.read_tls(io) {
                Ok(0) => return Err(SysError::ENOTCONN),
                Ok(_) => conn.process_new_packets().map_err(map_tls_error).map(|_| ())?,
                Err(err) if err.kind() == io::ErrorKind::WouldBlock => {
                    if crate::timer::get_time_us() >= deadline {
                        return Err(SysError::ETIMEDOUT);
                    }
    ...
    pub fn connect(fd: usize, host: &str) -> SyscallResult {
        ...
        if !matches!(tcp.lock().state, TcpSocketState::Established | TcpSocketState::CloseWait) {
            return Err(SysError::ENOTCONN);
        }
...
mod imp {
    ...
    pub fn connect(_fd: usize, _host: &str) -> SyscallResult {
        Err(SysError::ENOSYS)
    }

    pub fn write(_tls_id: usize, _buf: &[u8]) -> SyscallResult {
        Err(SysError::ENOSYS)
    }

    pub fn read(_tls_id: usize, _buf: &mut [u8]) -> SyscallResult {
        Err(SysError::ENOSYS)
    }

    pub fn close(_tls_id: usize) -> SyscallResult {
        Err(SysError::ENOSYS)
    }
}

pub use imp::{close, connect, read, write};
